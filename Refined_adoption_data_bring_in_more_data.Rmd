...
postcodes_with_coord <- read.csv("C:/Users/ouz001/book_chapter_data/Australian_Post_Codes_Lat_Lon.csv")
...
advisors_PA1 <- read_excel("C:/Users/ouz001/book_chapter_data/PA/PA_data_rawish.xlsx")
glimpse(advisors_PA)
...
XYadvisors_postcodes_join1 <- read.csv("C:/Users/ouz001/book_chapter_data/adoption_data/XYadvisors_postcodes_join.csv")
...
GRDC_RegionQ3 <- data.frame(count(XYadvisors_postcodes_join$GRDC_RegionQ3))
advisors_n_GRDC_region_ReZ <- data.frame(count(XYadvisors_postcodes_join$region_ReZoned))
glimpse(advisors_n_REGIONS)

#make the data frames neat and the names consistant (for joining)
glimpse(advisors_n_AGRO)
advisors_n_AGRO <- select(advisors_n_AGRO, zone = x,
                            number_of_farmers = freq) %>% 
                            mutate(grouping = "AGRO")
advisors_n_REGIONS <- select(advisors_n_REGIONS, zone = x,
                            number_of_farmers = freq) %>% 
                            mutate(grouping = "REGIONS")
advisors_n_state <- select(advisors_n_state, zone = x,
                            number_of_farmers = freq) %>% 
...
#advisors_n_GRDC_GRDC_RegionQ3 <- select(advisors_n_GRDC_Regio, zone = GRDC_RegionQ3,
#                            number_of_farmers = n) %>% 
#                            mutate(grouping = "GRDC_RegionQ3")

#advisors_n_GRDC_region_ReZ <- select(advisors_n_GRDC_region_ReZ, zone = region_ReZoned,
#                            number_of_farmers = n) %>% 
#                            mutate(grouping = "region_ReZ")
#glimpse(advisors_n_GRDC_Regio)
#glimpse(advisors_n_GRDC_region_ReZ)
...
step2_advisors_n_zones <- mutate(step1_advisors_n_zones, zone = as.factor(zone))
...
adoption_curve_function <- function(df, zone, numb_farmers) {
  ...
  count_adoption_year <- count(df_subset,df$year_as_factor)
  count_adoption_year <- select(count_adoption_year, year = df$year_as_factor, n)
...
#bring in the PA study
NoTill <- read.csv("C:/Users/ouz001/book_chapter_data/Weeds/no_till_adoption_curves_R.csv")
...
NoTill_n_Q2aRegion <- data.frame(count(XYNoTill_postcodes_join,Q2aRegion))
#NoTill_n_GRDC_region_ReZ <- data.frame(count(XYNoTill_postcodes_join,region_ReZ))
glimpse(NoTill_n_Q2aRegion)
...
NoTill_n_state <- select(NoTill_n_state, zone = state,
                            number_of_farmers = n) %>% 
                            mutate(grouping = "state")
NoTill_n_REGIONS_merge <- select(NoTill_n_Q2aRegion, zone = "Q2aRegion",
                            number_of_farmers = n) %>% 
                            mutate(grouping = "REGION_Merge")
#Append some of the data
NoTill_n_zones <- rbind(NoTill_n_AGRO,
                          NoTill_n_REGIONS,
                          NoTill_n_state,
                          NoTill_n_REGIONS_merge)
...
```{r adoption curve weeds study farmer per argozone MERGE etc, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}

#merge tas and high rainfall
glimpse(XYNoTill_postcodes_join)

#This works but I dont think I need it XYNoTill_postcodes_join has Q2aRegion
#XYNoTill_postcodes_join1 <- XYNoTill_postcodes_join %>% 
#                                  mutate(
#                                    test = case_when(
#                                   AGROECOLOG ==  "Vic High Rainfall" ~  "Vic high rainfall and Tas",
#                                   AGROECOLOG ==  "Tas Grain" ~  "Vic high rainfall and Tas",
#                                   TRUE ~ as.character(AGROECOLOG)))

#Bring in and modify the function

adoption_curve_function_NoTill_merge <- function(df, zone, numb_farmers) {
  df_subset <- filter(df, Q2aRegion == zone)
  count_adoption_year <- count(df_subset,year_as_factor)
  count_adoption_year <- select(count_adoption_year, year = year_as_factor, n)
  years_of_study <- data.frame(year = 1950:2014, 
                               id = 1:65,
...
Qld_Central_NT_M <- adoption_curve_function_NoTill_merge(XYNoTill_postcodes_join, "QLD CENTRAL", 28)
NSW_NorthEast_Qld_SouthEast_NT_M <- adoption_curve_function_NoTill_merge(XYNoTill_postcodes_join, "NSW NE - QLD SE", 45)

NSW_NorthWest_Qld_SouthWest_NT_M <- adoption_curve_function_NoTill_merge(XYNoTill_postcodes_join, "NSW NW- QLD SW", 46)

NSW_Central_NT_M     <- adoption_curve_function_NoTill_merge(XYNoTill_postcodes_join, "NSW CENTRAL", 49)
NSW_Vic_Slopes_NT_M <- adoption_curve_function_NoTill_merge(XYNoTill_postcodes_join, "NSW VIC SLOPES", 52)

SA_Midnorth_Lower_Yorke_Eyre_NT_M <- adoption_curve_function_NoTill_merge(XYNoTill_postcodes_join, "SA MIDNORTH - LOWER YORKE EYRE", 51)
SA_Vic_Bordertown_Wimmera_NT_M <- adoption_curve_function_NoTill_merge(XYNoTill_postcodes_join, "SA VIC BORDERTOWN – WIMMERA", 50)
SA_Vic_Mallee_NT_M <- adoption_curve_function_NoTill_merge(XYNoTill_postcodes_join, "SA VIC MALLEE", 52)

Vic_High_Rainfall_TAS_NT_M <- adoption_curve_function_NoTill_merge(XYNoTill_postcodes_join, "TAS GRAIN & VIC HIGH RAINFALL", 51)


WA_Central_NT_M <- adoption_curve_function_NoTill_merge(XYNoTill_postcodes_join, "WA CENTRAL", 45)
WA_Eastern_NT_M <- adoption_curve_function_NoTill_merge(XYNoTill_postcodes_join, "WA EASTERN", 47)
WA_Mallee_Sandplain_NT_M <- adoption_curve_function_NoTill_merge(XYNoTill_postcodes_join, "WA MALLEE AND SANDPLAIN", 46)
WA_Northern_NT_M <- adoption_curve_function_NoTill_merge(XYNoTill_postcodes_join, "WA NORTHERN", 40)

NoTill_adoption_Agro_NT_merge <- rbind(Qld_Central_NT_M, 
                                       NSW_NorthEast_Qld_SouthEast_NT_M, 
                                       NSW_NorthWest_Qld_SouthWest_NT_M,
                                       NSW_Central_NT,
                          NSW_Vic_Slopes_NT_M,
                          SA_Midnorth_Lower_Yorke_Eyre_NT_M,
                          SA_Vic_Bordertown_Wimmera_NT_M,
                          SA_Vic_Mallee_NT_M,
                          Vic_High_Rainfall_TAS_NT_M,
                          WA_Central_NT_M,
                          WA_Eastern_NT_M,
                          WA_Mallee_Sandplain_NT_M,
                          WA_Northern_NT_M) 
...
```{r plot NoTill Agro merge, echo=FALSE}
Q2aRegion <- c(
`QLD CENTRAL` = "Qld Central",
`NSW NE - QLD SE` = "NSW NE/Qld SE",
`NSW NW- QLD SW` = "NSW NW/Qld SW",
`NSW CENTRAL` = "NSW Central",
`NSW VIC SLOPES` = "NSW VIC Slopes",
`SA MIDNORTH - LOWER YORKE EYRE` = "SA Midnorth - Lower Yorke Eyre",
`SA VIC BORDERTOWN – WIMMERA` = "SA VIC Bordertown - Wimmera",
`SA VIC MALLEE` = "SA VIC Mallee",
`TAS GRAIN & VIC HIGH RAINFALL` = "VIC High Rainfall and TAS Grain",
`WA CENTRAL` = "WA Central",
`WA EASTERN` = "WA Eastern",
`WA MALLEE AND SANDPLAIN` = "WA Sandplain - Mallee",
`WA NORTHERN` = "WA Northern")

glimpse(Q2aRegion)
print(NoTill_n_zones[26,1])

ggplot(NoTill_adoption_Agro_NT_merge, aes(year, cumm_percent, group=zone))+
  geom_line()+
  theme_classic()+
  theme(legend.position = "bottom",
  ...
  facet_wrap(.~zone, labeller = labeller(zone = label_wrap_gen()))+
  #facet_wrap(.~zone, labeller = labeller(zone = Q2aRegion, label_wrap_gen()))+
  labs(x = "",
       y = "Percenatge of farmers in year first trying no-till") #,
       #title = "Adoption of no till use per AgroEcological")


ggsave("NoTill_adoption_Agro_NT_merge.png", width = 9.8, height = 5.6, units = "in")
...
```{r plot NoTill region overlayed with national, echo=FALSE}

glimpse(NoTill_adoption_Region_NT)
glimpse(National_NT)
National_NT_for_graph <- National_NT %>% 
                          mutate(zone = "national")
glimpse(National_NT_for_graph)
National_Region_NT_for_graph <- rbind(National_NT_for_graph,NoTill_adoption_Region_NT)
glimpse(National_Region_NT_for_graph)

ggplot(National_Region_NT_for_graph, aes(year, cumm_percent, group = zone))+
  geom_smooth(data = subset(National_Region_NT_for_graph, zone == "national"),se=FALSE, colour = "black")+
  geom_line(data = subset(National_Region_NT_for_graph, zone != "national"), aes(linetype=zone) )+
  ...
  labs(x = "",
       y = "Percenatge of farmers in year first trying no-till" )
       #title = "Adoption of no till use per Region")
ggsave("NoTill_adoption_Region_with_national.png", width = 9.8, height = 5.6, units = "in")
...
cultivation_prior_seeding <- read.csv("C:/Users/ouz001/book_chapter_data/Weeds/cultivation.csv")
...
#bring in the windrow burning data with zone info merged 
windrow_burn_adoption <- read.csv("C:/Users/ouz001/book_chapter_data/Weeds/Raw_data_Weeds_with_postcodes_windrow_burn.csv")
...
No_Till_perception <- read_excel("C:/Users/ouz001/book_chapter_data/Weeds/Raw_data_Weeds_with_postcodes.xlsx", sheet= "GRDC_Weed_in_Crops_rawish")
